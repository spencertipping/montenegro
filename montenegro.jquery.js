...
//   Some events are common enough that it's useful to have a handler for them. Hitting the enter key is one of those:
    ...
    tconfiguration('std', 'montenegro.jquery.events', function () {$.fn.enter(f) = this.keyup(fn[e][e.detail === 13 && f.call(this, e)])}).
...
// Data modeling.
// Mapping data onto UI components is not always straightforward. If the UI just represents an object (i.e. no arrays), then using the .val() method is generally an easy solution. The problem
// comes in when you have arrays of elements or any irregular data requirements.

// Final configuration.
// This one loads all of the others.

  configuration('montenegro.jquery', function () {this.configure('montenegro.jquery.events montenegro.jquery.dom')});
