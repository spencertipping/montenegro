...
// interfaces instead of going the HTML route. For logic-heavy applications this may make more sense than separating the layout.
  ...
  caterwaul.configuration('montenegro.jquery.core', function () {this.shallow('montenegro', {jquery: {$: jQuery}})}).
...
//   a good thing. The markup should be simple and local, and your modifier functions should be short enough to type several times. (This can be achieved by using a let-binding or similar.)
    ...
    tconfiguration('std seq', 'montenegro.jquery.dom', function () {var jquery = this.configure('montenegro.jquery').montenegro.jquery, $ = jquery.$;
      let*[create_node(tag)     = qs[$(document.createElement(_tag))].replace({_tag: new this.ref(tag)}),
           ...
           attributes(e, attrs) = qs[_e.se(fn_[_as])].
                                  replace({_e: new this.ref(e), _as: attrs.flatten(',').map(fn[a][qs[this.attr(_name, _value)].replace({_name: new this.ref(a[0].data), _value: a[1]})])}),
      
           expander(expression) = let[m = expression.match(qs[_/_])][m ? qs[_e._f].replace({_e: expander(m[0]), _f: m[1]}) :
                                      (m = expression.match(qs[_._]))  ? qs[_e.addClass(_c)].replace({_e: expander(m[0]), _c: new this.ref(m[1].data)}) :
                                      (m = expression.match(qs[_%_]))  ? qs[_f(_e)].replace({_e: m[0], _f: m[1]}) :
                                      (m = expression.match(qs[_[_]])) ? qs[_f(_e, _c)].replace({_f: new this.ref(append), _e: expander(m[0]), _f: expander(m[1])}) :
                                      (m = expression.match(qs[_(_)])) ? attributes(m[0], m[1]) :
                                      (m = expression.match(qs[!_]))   ? m[0] : create_node(m[0])]] in
...
// This one loads all of the others.
  ...
  configuration('montenegro.jquery', function () {this.configure('montenegro.jquery.core montenegro.jquery.dom')});
