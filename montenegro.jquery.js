...
//   Aliases are transitive, but this dependency is resolved at definition-time, not as the aliases are used. (So if you do something inadvisable such as changing an alias, the transitivity
//   relation won't make much sense.)
    ...
    tconfiguration('std seq iter', 'montenegro.jquery.dom', function () {
      ...
      let*[as = this.montenegro.html_aliases = {}, a = this.montenegro.html_alias(e, t) = (as[e] = as[t] || t, this)]
      [this.macro(qs[_ >html(alias)> _], fn[e, t][qs[_f(_e, _t)].replace({_f: new this.ref(a), _e: new this.ref(e.data), _t: new this.ref(t.data)})]),
       ...
       where*[ref(x)                 = new caterwaul.ref(x),
              node_create(tag)       = qs[_$(_document.createElement(_as[_tag] || _tag))].replace({_$: ref(jQuery), _as: ref(as), _tag: ref(tag), _document: ref(document)}),
