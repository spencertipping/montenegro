...
//   (http://en.wikipedia.org/wiki/Kerckhoffs'_principle) into the framework to prevent bad security decisions.
      ...
      _.rpc.testpage() = html(qs[$('head').append(html[style('body {font-family: sans-serif; font-size: 9pt; width: 800px; margin: auto} a {color: #35a; cursor: pointer} .error {color: red} ',
                                                             'textarea {border: solid 1px #ccc; min-height: 200px; min-width: 800px} div.log {border-top: solid 1px #ccc} ')]),
                                 $('body').append(html[div > div.header(h1('RPC shell for ', code(url)), div.documentation)
                                 ...
                                 $.get(url, fn[doc][$('div.documentation').append(doc)]),

                                 where*[entry(x) = html[div.entry(x, a('[x]')/click(fn_[$(this).parent().remove()]))],
