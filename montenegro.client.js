...
//   in a first-class way (which would normally be necessary to set the cycle up again).
    ...
    tconfiguration('std', 'montenegro.jquery.rpc', function () {
      this.configure('montenegro.jquery.core').montenegro
      }).
    ...
    tconfiguration('std seq iter', 'montenegro.jquery.dom', function () {
      ...
      [this.macro(qs[_ >html(alias)> _], fn[e, t][qs[_f(_e, _t)].replace({_f: new this.ref(a), _e: new this.ref(e.data), _t: new this.ref(t.data)})]),
       ...
       where*[ref(x)                 = new caterwaul.ref(x),
              ...
              append_multiple(node)  = (x !== null && x !== undefined && append_single(node, x)) <se< sa<< Array.prototype.slice.call(arguments, 1) && node,
